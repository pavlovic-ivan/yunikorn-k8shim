...
func TestMutate(t *testing.T) {
	...
	assert.Equal(t, annotations[common.UserInfoAnnotation].(string), "{\"user\":\"testExtUser\"}") //nolint:errcheck
...
func schedulerName(t *testing.T, patch []byte) string {
	...
	for _, op := range ops {
		if op.Path == "/spec/schedulerName" {
			return op.Value.(string) //nolint:errcheck
		}
	}
	return ""
}
...
func labels(t *testing.T, patch []byte) map[string]interface{} {
	...
	for _, op := range ops {
		if op.Path == "/metadata/labels" {
			return op.Value.(map[string]interface{}) //nolint:errcheck
		}
	}
